...
async def query(
    ...
    atletas = (await db_session.execute(query.limit(limit).offset(offset))).scalars().all()
...
async def query(
    ...
    query = select(AtletaModel)
    atletas = (await db_session.execute(query.limit(pagination.limit).offset(pagination.offset))).scalars().all()

    return Page[AtletaOut](items=atletas, total=len(atletas))
